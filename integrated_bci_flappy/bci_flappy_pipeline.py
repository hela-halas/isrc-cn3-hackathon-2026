...
def infer_label(path: Path, positive_classes: set[str]) -> int | None:
    ...
    if "leg" in name:
        return 1 if "legs" in positive_classes or "leg" in positive_classes else None
    return None


def signal_columns(df: pd.DataFrame) -> list[str]:
    numeric_cols = [col for col in df.columns if pd.api.types.is_numeric_dtype(df[col])]
    return [col for col in numeric_cols if str(col).strip().lower() not in DEFAULT_EXCLUDE_COLUMNS]
...
def build_training_set(args) -> tuple[np.ndarray, np.ndarray, np.ndarray, FeatureConfig, list[dict]]:
    positive_classes = set(parse_csv_list(args.positive_classes) or ["hands", "legs"])
    csv_paths = sorted(Path(args.data_dir).glob("*.csv"))
    if not csv_paths:
    ...
    for path in csv_paths:
        if stream_header_is_demo(path) and not args.include_demo:
            manifest.append({"file": path.name, "status": "skipped", "reason": "demo_eeg stream"})
            continue
        label = infer_label(path, positive_classes)
        if label is None:
            manifest.append({"file": path.name, "status": "skipped", "reason": "no selected label"})
            continue
        ...
        if first_channels is None:
            ...
            raise ValueError(
                f"{path.name} selected channels {channels}, but earlier files used {first_channels}. "
                "Pass --channels explicitly if the column names differ."
            )

        feats = window_features(data, fs, placeholder_cfg)
        X_parts.append(feats)
        y_parts.append(np.full(feats.shape[0], label, dtype=int))
        group_parts.append(np.full(feats.shape[0], path.name, dtype=object))
        manifest.append(
            {
                "file": path.name,
                "status": "used",
...
def realtime_command(args) -> int:
    ...
    try:
        while True:
            ...
            score = prediction_score(model, extract_features(data, fs, cfg))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument("--positive-classes", default="hands,legs", help="classes mapped to flap: hands, legs, or both")
