...
def extract_features(data: np.ndarray, fs: float) -> np.ndarray:
    ...
    for ch_idx in range(data.shape[0]):
        ...
        valid_freqs = np.logical_and(freqs >= 1.0, freqs <= 50.0)
        total_power = np.sum(psd[ch_idx, valid_freqs])
	
        if np.isnan(total_power) or total_power < 1e-10:
            total_power = 1.0
        ...
        for band_name, (low, high) in bands.items():
            ...
            abs_power = np.sum(psd[ch_idx, band_idx])
...
def get_trained_classifier(fs: float, window_sec: float, channels: np.ndarray, n_features: int):
    ...
    for label, filename in calibration_files.items():
        ...
        clean_data = apply_bandpass_filter(raw_data, fs, lowcut = 1.0, highcut = 60.0)
        ...
        for start_idx in range(0, total_samples - window_samples + 1, step_samples):
            ...
            features = np.nan_to_num(features, nan=0.0, neginf=0.0)
...
def grab(args):
    ...
    # train classifier 
    n_features = n_channels * 4 # theta, alpha, beta per channel
    ...
    try:
        while True:
            ...
            # Step 1: preprocess
            clean_epoch = apply_bandpass_filter(epoch, fs, lowcut = 1.0, highcut=60.0)
            ...
            outlet.push_sample([unity_value])
